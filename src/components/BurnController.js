    ...
    <div className="space-y-3">
      ...
      <div className="flex space-x-2">
        ...
        <button
          ...
          className="px-6 py-2 rounded-xl bg-gradient-to-r from-orange-500/80 to-red-500/80 text-white font-bold hover:from-orange-400 hover:to-red-400 disabled:opacity-50 transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-orange-400/25 backdrop-blur-sm border border-orange-400/30 relative overflow-hidden group flex items-center"
        >
          {loading ? (
            <span className="loader mr-2 w-4 h-4 border-2 border-white border-t-orange-400 rounded-full animate-spin"></span>
          ) : null}
      ...
      {burnAmount && parseFloat(burnAmount) > parseFloat(balance) && (
        <div className="text-xs text-red-400 mt-2 p-2 rounded-lg bg-red-500/10 border border-red-400/20 animate-pulse">
          ⚠️ Insufficient token balance to burn.
        </div>
      )}
    </div>
  );
